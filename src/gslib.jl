@with_kw mutable struct GSLIBDistribution
    ...
    esmda_update::Union{Nothing, Matrix{Float64}} = nothing
end
...
function sample_sgsim(p::GSLIBDistribution, dir="sgsim_output/")
    # println("I'm in sample sgsim! Thank you!")
    ...
    # save_dir_prefix = "/home/ccs/True_porosity/"
    # poro = deserialize(string(save_dir_prefix,"porosity.dat"));
    # perm = deserialize(string(save_dir_prefix,"permeability.dat"));
    # return poro, perm, perm
end

function Base.rand(p::GSLIBDistribution, dir="sgsim_output/"; silent::Bool=true)
    # println("I'm in rand! Thank you!")
    # Write the param file
    if isnothing(p.esmda_update)
        if silent
            stdout_orig = stdout
            redirect_stdout()
        end
        ...
        sampled_MC = rand(1:size(p.esmda_update)[2]);
        vals = p.esmda_update[:,sampled_MC];
        poro_2D = reshape(vals, (80,80,1));
        porosity = repeat(poro_2D, outer=(1, 1, 8));
        permeability = porosity.^3.0*(1e-5).^2.0./(0.81*72.0*(1.0.-porosity).^2.0);

        return porosity, permeability, permeability
    end
